...
function Lobby({ socket, state, managerId, setError, leave }: { socket: GameSocket; state: RoomState; managerId: string; setError: (value: string) => void; leave: () => void }) {
  ...
  const changeBlindRevealStyle = (blindRevealStyle: "blur" | "wipe") => updateSettings({ blindRevealStyle });
...
function getBlindDirectionalClipPath(direction: BlindPublicState["revealDirection"], progress: number): string {
  const clamped = Math.max(0, Math.min(1, progress));
  const hiddenPercent = (1 - clamped) * 100;
  switch (direction) {
    case "bottom-up":
      return `inset(${hiddenPercent}% 0 0 0 round 22px)`;
    case "left-right":
      return `inset(0 ${hiddenPercent}% 0 0 round 22px)`;
    case "right-left":
      return `inset(0 0 0 ${hiddenPercent}% round 22px)`;
    case "top-down":
    default:
      return `inset(0 0 ${hiddenPercent}% 0 round 22px)`;
  }
}

function BlindRevealImage({ blind, revealStyle = "blur" }: { blind: BlindPublicState; revealStyle?: "blur" | "wipe" }) {
...
function FormationRoom({ socket, state, managerId, setError, leave }: { socket: GameSocket; state: RoomState; managerId: string; setError: (value: string) => void; leave: () => void }) {
  ...
  return <main className="formation-page page"><header className="formation-header"><div><div className="eyebrow">POST-AUCTION TEAM SETUP</div><h1>Choose formation & assemble your team</h1><p>Tap two players to swap, or drag with mouse, touch, or stylus. Primary and supported roles receive the best rating.</p></div><FormationClock endsAt={state.formationEndsAt} /></header>
    <div className="formation-layout"><aside className="panel formation-list"><div className="panel-title"><h2>Formations</h2><span>{validFormations.length} OPTIONS</span></div><div className="formation-options">{validFormations.map(item => <motion.button whileTap={{ scale: .96 }} whileHover={{ scale: 1.015 }} className={formationId === item.id ? "active" : ""} onClick={() => chooseFormation(item.id)} key={item.id}><b>{item.name}</b><span>{item.style}</span></motion.button>)}</div></aside>
...
function RoomChat({ socket, state, managerId, setError }: { socket: GameSocket; state: RoomState; managerId: string; setError: (value: string) => void }) {
  ...
  return <div className={`room-chat phase-${state.phase} ${open ? "open" : ""}`}>
    {open && <motion.section className="chat-window" style={{ x: chatX, y: chatY }} aria-label="Room chat">
    ...
    <motion.button className="chat-launcher" style={{ x: chatX, y: chatY, touchAction: "none" }} drag dragMomentum={false} dragElastic={0.05} dragConstraints={dragConstraints} onDragEnd={persistChatPosition} onDoubleClick={resetChatPosition} onClick={() => open ? closeChat() : setOpen(true)} aria-label={open ? "Close room chat" : "Open room chat"} title="Drag to move · double-click to reset position"><span>{open ? "×" : "💬"}</span>{!open && unread > 0 && <b>{Math.min(unread, 99)}</b>}</motion.button>
  </div>;
}
